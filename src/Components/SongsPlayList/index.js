...
class SongsPlayList extends Component {
  state = {final: initialTracksList, value: ''}
  ...
  render() {
    const {final, value} = this.state
    const ans = final.filter(each =>
    ...
    return (
      <div>
        <div className="FirstContainer">
          ...
          <p className="description">Singer</p>
        </div>
        <div className="secondContainer">
          <div className="insideSecondContainer">
            <h1 className="secondContainerHeading">Songs Playlist</h1>
          ...
          {ans.length > 0 ? (
            <ul>
              {ans.map(each => (
                <Final user={each} key={each.id} deleteTodo={this.deleteTodo} />
              ))}
            </ul>
          ) : (
            <div className="tharunContainer">
              <p className="tharun">No Songs Found</p>
            </div>
          )}
        </div>
      </div>
